...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadCongViec() {
        ...
        try {
            ...
            if (role !== "admin") {
                data = data.filter(item => item.tenNhanVien === user.ten);
            }
    ...
    document.getElementById("searchCongViec")?.addEventListener("input", function () {
        ...
        document.querySelectorAll("#congViecList .item").forEach(item => {
            item.style.display =
    ...
    addBtn?.addEventListener("click", async () => {
        ...
        if (!tieuDe || !han || !nhanVienId) {
            message.textContent = "Vui lòng nhập đủ tiêu đề, hạn và chọn nhân viên";
            return;
        }
