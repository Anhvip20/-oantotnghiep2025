...
if (showAddBtn) {
    showAddBtn.addEventListener("click", () => {
        if (formCard.style.display === "none") {
            ...
            showAddBtn.textContent = "Đóng form thêm nhân viên";
        } else {
            formCard.style.display = "none";
            showAddBtn.textContent = "Thêm nhân viên";
            resetNhanVienForm();
        }
    });
}
    ...
    try {
        ...
        box.innerHTML = data.map(item => `
            <div class="item">
                ...
                <button class="danger" onclick="deleteNhanVien(${item.id})">Xóa</button>
                ` : ""}
            </div>
        `).join("");
    } catch (error) {
...
if (searchInput) {
    searchInput.addEventListener("input", function () {
        ...
        const items = document.querySelectorAll("#nhanVienList .item");
        items.forEach(item => {
...
function editNhanVien(id, ten, email, matkhau, role) {
    ...
    document.getElementById("nhanVienFormTitle").textContent = `Đang sửa nhân viên ID ${id}`;
    document.getElementById("addNhanVienBtn").textContent = "Cập nhật nhân viên";
    document.getElementById("cancelNhanVienBtn").style.display = "inline-block";

    setTimeout(() => document.getElementById("tenNhanVien").focus(), 300);
}
window.editNhanVien = editNhanVien;

document.getElementById("addNhanVienBtn").addEventListener("click", saveNhanVien);
document.getElementById("cancelNhanVienBtn").addEventListener("click", cancelEditNhanVien);

async function saveNhanVien() {
    const ten = document.getElementById("tenNhanVien").value.trim();
    const email = document.getElementById("emailNhanVien").value.trim();
    ...
    try {
        ...
        if (editingNhanVienId) {
            response = await fetch(`${API_BASE}/nhanvien/${editingNhanVienId}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ten, email, matkhau, role })
            });
        } else {
            response = await fetch(`${API_BASE}/nhanvien`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ten, email, matkhau, role })
            });
        ...
        if (!response.ok) {
            message.textContent = data.message || "Lưu nhân viên thất bại";
            return;
        }

        message.textContent = editingNhanVienId
...
async function deleteNhanVien(id) {
    const ok = confirm(`Bạn có chắc muốn xóa nhân viên ID ${id} không?`);
    if (!ok) return;

    try {
        const response = await fetch(`${API_BASE}/nhanvien/${id}`, { method: "DELETE" });
        const data = await response.json();
        if (!response.ok) {
...
function resetNhanVienForm() {
    ...
    document.getElementById("cancelNhanVienBtn").style.display = "none";
    document.getElementById("nhanVienFormTitle").textContent = "Thêm nhân viên mới";
}

function cancelEditNhanVien() {
    resetNhanVienForm();
    document.getElementById("nhanVienMessage").textContent = "Đã hủy chỉnh sửa";
}
